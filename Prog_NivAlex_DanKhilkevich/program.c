...
void FromFile2Sportsman(const char* filename, sportsman* sportsmen_array[], int sportsmen_array_size) 
{
    ...
    *sportsmen_array = malloc(sportsmen_array_size * sizeof(sportsman));  // allocate memory for sportsmen array;
    ...
    int count = 0;
    while (fgets(line, sizeof(line), file) != NULL && count < sportsmen_array_size)  //read line by line from the file 
    {
        int gen;
        char clubName[100]; // temporary buffer for club name, club name = maximum of 100 chars

        if (sscanf(line, "%d;%[^;];%[^;];%[^;];%d", &((*sportsmen_array)[count].id),(*sportsmen_array)[count].Fname,
            (*sportsmen_array)[count].Lname,clubName,&gen) != 5) 
        {
            printf("Error reading data from file\n");
            exit(1);
        }
        (*sportsmen_array)[count].gen = gen;

        // dynamically allocate memory to p2club based on the length of clubName
        (*sportsmen_array)[count].p2club = malloc((strlen(clubName) + 1) * sizeof(char));
        if ((*sportsmen_array)[count].p2club == NULL) 
        {
            printf("Memory allocation failed\n");
            exit(1);
        }
        strcpy((*sportsmen_array)[count].p2club, clubName);
        count++;
    }
...
void main() {
    ...
    printf("Sportsman Data:\n");
    for (int i = 0; i < numSportsmen; i++) 
    {
        printf("ID: %d; Name: %s %s; Club: %s; Gender: %s\n", sportsmen[i].id,
            sportsmen[i].Fname, sportsmen[i].Lname, sportsmen[i].p2club,
            (sportsmen[i].gen == 1) ? "Male" : "Female");

       
        free(sportsmen[i].p2club); // free memory allocated for p2club
    }
